...
//2-UL(Pin2)  4-VL(Pin3)  6-WL;(Pin4) //旧版
...
#define _MaxIshunt 600  //Limit Normal run current about 12A  ( the resistance value 10momega)
#define _MaxUpIshunt 580 //Limit Startup Current about 3A
#define _MaxMaxIshunt 680 //Limit Peeak Current about 16A 
...
void setup(){
  Serial.begin(9600); 
  pinMode(2, OUTPUT);  //U-L
  pinMode(3, OUTPUT);  //V-L
  pinMode(4, OUTPUT);  //W-L
...
void CheckInputPin(){
    ...
    if(RunEnable!=RunPin){
       RunPinChangeCount++;
       if(RunPinChangeCount>_KeyMaxCount){ RunEnable=RunPin;RunPinChangeCount=0;}
    }
    else{
      RunPinChangeCount=0;
    }
